...
export class CustomerStaffCmsComponent implements OnInit {
  ...
  constructor(
    private service: CustomerStaffCmsService,
  ...
  getstaffimage() {
    this.service.getstaffprofilepicture(this.crffnmasterid).subscribe(
      result => {
  ...
  staffinformation() {
    ...
    for (let x of this.stafflist) {
      const found = this.staffImage
        .filter(y => y.staffid === x.staffid)
        .shift();
      if (found) staffinformation.push({ ...x, ...found });
    }
    staffinformation.push(
      ...this.staffImage.filter(
        z => this.stafflist.map(x => x.staffid).indexOf(z.staffid) === -1
      )
    );
  }
  ...
  deleteStaff(staffid) {
    if (window.confirm("Are you sure you want to delete?")) {
      this.service.deletestaff({ staffid: staffid }).subscribe(
        result => {
  ...
  createStaff() {
    ...
    this.service.createstaff(this.data).subscribe(
      result => {
