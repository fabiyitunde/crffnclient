...
export class ViewimagesComponent implements OnInit {
  ...
  aboutuslist: any[] = [];
  p: number = 1;
  constructor(private service: ViewimagesService, private route: ActivatedRoute, private router: Router) { }
  ...
  getstafflist() {
    ...
    this.service.getAboutUsList(this.crffnmasterid).subscribe(result => {
      ...
      this.aboutuslist = list.filter(item => item.aboutusitemtype === "Picture");
