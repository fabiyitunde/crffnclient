...
export class ApproveinternalinvoiceComponent implements OnInit {
  invoicelist: any[] = [];
  p: number = 1;
  constructor(private service: ApproveInternalInvoiceService, private route: ActivatedRoute, private router: Router, private modalService: NgbModal) { }
  ...
  approveInvoice(invoiceid) {
    ...
    if (window.confirm('Are you sure you want to approve?')) {
      this.service.approveInvoice({ invoicemasterid: invoiceid }).subscribe(result => { }, err => {
        alert(err);
      })
      this.getPendingInvoiceList();
    }
  ...
  returnInvoice(invoiceid) {
    ...
    if (window.confirm('Are you sure you want to return invoice?')) {
      this.service.returnInvoice({ invoicemasterid: invoiceid }).subscribe(result => { }, err => {
        alert(err);
      })
      this.getPendingInvoiceList();
    }
